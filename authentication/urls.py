...
from authentication.views import index_view, login_modal, discord_link, exchange_code_for_access_token, discord_login, discord_login_callback, discord_register, discord_register_callback

app_name = 'authentication'
urlpatterns = [
    path('', index_view, name="index"),
    path('login/', login_modal, name='login'),
    #path('register/', UserRegisterModalView.as_view(), name='register'),
    # path('accounts/discord/link/', discord_link, name="discord_link"), # We bypass this via the below callback
    # path('accounts/discord/link/callback/', exchange_code, name="discord_link_callback"),
